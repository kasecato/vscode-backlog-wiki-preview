...
export class DynamicMarkdownPreview extends Disposable {
	...
	public static readonly viewType = 'backlog.preview';
		...
		this._register(this.editor.webview.onDidReceiveMessage((e: CacheImageSizesMessage | RevealLineMessage | DidClickMessage | ClickLinkMessage | ShowPreviewSecuritySelectorMessage | PreviewStyleLoadErrorMessage) => {
			...
			switch (e.type) {
				...
				case 'showPreviewSecuritySelector':
					vscode.commands.executeCommand('backlog.showPreviewSecuritySelector', e.source);
					break;

				case 'previewStyleLoadError':
					vscode.window.showWarningMessage(localize('onPreviewStyleLoadError', "Could not load 'backlog.styles': {0}", e.body.unloadedStyles.join(', ')));
					break;
			}
		}));
	...
	private async onDidClickPreviewLink(href: string) {
		...
		vscode.commands.executeCommand('_backlog.openDocumentLink', { path: hrefPath, fragment, fromResource: this.resource });
	}
}
